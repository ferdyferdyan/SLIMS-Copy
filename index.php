...
if (isset($_POST['act']) && $_POST['act'] == 'save' && isset($_POST['p2precord'])) {
    ...
    $msg_box = '<div class="alert alert-success">Berhasil menyalin <b>'.$count.'</b> buku ke database lokal!</div>';
}
...
if (isset($_POST['act']) && $_POST['act'] == 'search') {
    ...
    foreach ($sysconf['p2pserver'] as $idx => $srv) {
        ...
        if ($data && isset($data['records']) && count($data['records']) > 0) {
            ...
            $badge = '<span class="badge badge-light text-dark">'.$count_rec.' Buku</span>';
        }
        ...
        if ($found_in_this_server) {
            ...
            $table->setHeader(['Pilih', 'Cover', 'Judul', 'Detail']);
            ...
            // Pesan kosong
            $server_card_html .= '<div class="p-2 text-muted font-italic text-center small">Tidak ada buku ditemukan.</div>';
        }
    ...
    if ($total_found_global > 0) {
        $msg_box = '<div class="alert alert-info">Total ditemukan <b>'.$total_found_global.'</b> buku dari seluruh server.</div>';
        $results_html .= '<div class="p-3 bg-light sticky-top shadow-sm mb-3" style="z-index:99; border-bottom:1px solid #ccc; top:0;">
                            <button type="submit" class="s-btn btn btn-success btn-lg shadow">
                                <i class="fa fa-save"></i> Simpan Data Terpilih
                            </button>
                          </div>';
    } else {
        $msg_box = '<div class="alert alert-warning mt-3">Tidak ada buku ditemukan di server manapun.</div>';
    }
    ...
    if (!empty($html_found) && !empty($html_empty)) {
        $results_html .= '<div class="text-center mt-4 mb-2"><span class="badge badge-secondary p-2">Server Tanpa Hasil</span></div>';
    }
...
<div class="menuBox">
    <div class="menuBoxInner biblioIcon">
        <div class="per_title"><h2>SLiMS Copy (Multi-Search)</h2></div>
        ...
        <div class="sub_section">
            <form action="<?php echo $current_action_url; ?>" method="post" class="form-inline">
                ...
                <label class="mr-2">Kata Kunci:</label>
                <input type="text" name="keywords" class="form-control" style="width:300px;" value="<?= isset($_POST['keywords'])?$_POST['keywords']:'' ?>" required placeholder="Ketik judul buku...">
                <button type="submit" class="btn btn-primary ml-2">Cari di Semua Server</button>
            </form>
        </div>
